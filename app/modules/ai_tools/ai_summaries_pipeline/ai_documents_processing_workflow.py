...
from app.modules.ai_tools.schemas import ProcurementDocument
    ...
    async def process_tender(
        ...
        try:
            ...
            for doc_id in markdown_contents.keys():
                # Always use the original filename from the download for consistency
                # This ensures the same filename is used throughout the process
                _, _, original_filename = pdf_data[doc_id]
                pdf_paths[doc_id] = original_filename

            self.logger.info(f"PDF paths for chunking: {pdf_paths}")
            # Process markdown contents directly
            all_flat_chunks = self.markdown_chunking_service.chunk_markdown_contents(markdown_contents, pdf_paths)
            ...
            # Upload PDF files to Azure
            self.logger.info("Step 4.1: Uploading PDF documents to Azure")
            pdf_azure_paths = {}
            for doc_id, (temp_path, content, original_filename) in pdf_data.items():
                # Upload PDF content directly to Azure using the specialized method
                try:
                    blob_path = self.azure_client.upload_tender_pdf(output_id, content, original_filename)
                    pdf_azure_paths[doc_id] = blob_path
                    self.logger.info(f"Uploaded PDF {original_filename} to Azure: {blob_path}")
                except Exception as e:
                    self.logger.error(f"Failed to upload PDF {original_filename}: {str(e)}")
            ...
            return {
                ...
                "pdf_paths": pdf_azure_paths if 'pdf_azure_paths' in locals() else None,
            }
