...
from tests.queries_tender_detail import (query_core_template, query_identifier,
                                        query_lots, query_contracting_entity, query_monetary_values,
                                        query_contractual_terms_and_location,
                                        query_cpvs, query_submission_terms, query_legal_documents,
                                        query_technical_documents, query_additional_documents)
...
async def test_async_neptune_queries(tender_hash: str):
    # Get Neptune client
    neptune = get_neptune_client()
    
    if tender_hash.startswith("http"):
        tender_uri = tender_hash
    else:
        tender_uri = f"http://gober.ai/spain/procedure/{tender_hash}"
    ...
    logger.info("Executing named SPARQL queries in parallel...")
    
    try:
        start_time = time.time()
        
        named_results = await neptune.execute_named_sparql_queries_parallel(named_queries)
        
        execution_time = time.time() - start_time
        logger.info(f"Named queries execution time: {execution_time:.2f} seconds")
        
        tender_detail = parse_tender_detail(named_results)
        
        logger.info(f"Tender detail: {tender_detail}")
        
        return tender_detail
        
    except Exception as e:
        logger.error(f"Error executing named queries: {str(e)}")
...
if __name__ == "__main__":
    ...
    asyncio.run(test_async_neptune_queries("bc1ac0325a7cd12cfecfbf3077fb895799920d6ab756e391abfba3b79f1b6579"))
